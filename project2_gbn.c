#include "project2_gbn.h"

/* ******************************************************************
 GO-BACK-N NETWORK EMULATOR: VERSION 1.1  J.F.Kurose
...
void A_output(struct msg message)
{
	...
	if (A.next_seq_num < A.base + WIN_SIZE)
	{

		// Create the outgoing packet
		struct pkt new_packet;
		new_packet.seqnum = A.next_seq_num;
		memcpy(new_packet.payload, message.data, 20);
		new_packet.checksum = calculate_checksum(new_packet);

		// Insert it into the sending window and send it
		A.pkt_window[A.next_seq_num % WIN_SIZE] = new_packet;
...
void B_output(struct msg message){
	// Currently in the waiting state so we can send a new packet
	if (B.next_seq_num < B.base + WIN_SIZE)
	{

		// Create the outgoing packet
		struct pkt new_packet;
		new_packet.seqnum = B.next_seq_num;
		memcpy(new_packet.payload, message.data, 20);
		new_packet.checksum = calculate_checksum(new_packet);

		// Insert it into the sending window and send it
		B.pkt_window[B.next_seq_num % WIN_SIZE] = new_packet;
...
int main()
{
	...
	printf(" %d successful ACKs were transferred between the hosts\n",successful_acks);
	return 0;
}
